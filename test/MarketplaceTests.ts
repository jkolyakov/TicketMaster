...
describe("TicketMarketplace", function () {
  ...
  async function deployMarketplaceERC20() {
    ...
    const ticketNFT = await ethers.getContractAt(
      "TicketNFT",
      await ticketMarketplace.nftContract()
    );
    return { owner, ticketMarketplace, sampleCoin, ticketNFT, notOwner };
  }

  async function marketplaceWithEvents() {
    const { owner, ticketMarketplace, sampleCoin, ticketNFT, notOwner } =
      await deployMarketplaceERC20();
    ...
    await sampleCoin
      .connect(owner)
      .transfer(await notOwner.getAddress(), ethers.parseUnits("1", 18));
    await sampleCoin
      .connect(notOwner)
      .approve(
        await ticketMarketplace.getAddress(),
  ...
  describe("Deployment", function () {
    it("Should set the nftContract address", async function () {
      const { ticketNFT, ticketMarketplace } = await loadFixture(
        deployMarketplaceERC20
      );
      expect(await ticketMarketplace.nftContract()).to.be.eq(
        await ticketNFT.getAddress()
      );
    });

    it("Should set the ERC20Address address", async function () {
      const { sampleCoin, ticketMarketplace } = await loadFixture(
        deployMarketplaceERC20
      );
      expect(await ticketMarketplace.ERC20Address()).to.be.eq(
        await sampleCoin.getAddress()
      );
    });

    it("Should correctly set the owner of the marketplace", async function () {
      const { owner, ticketMarketplace } = await loadFixture(
        deployMarketplaceERC20
      );
    ...
    it("Should correctly set the owner of the NFT contract", async function () {
      const { ticketMarketplace, ticketNFT } = await loadFixture(
        deployMarketplaceERC20
      );
      expect(await ticketNFT.owner()).to.equal(
        await ticketMarketplace.getAddress()
      );
    });
    ...
    it("Should mint ERC20 tokens to the owner of the coin contract", async function () {
      ...
      expect(await sampleCoin.balanceOf(owner)).to.be.eq(
  ...
  describe("createNewEvent", function () {
    it("Should revert when not called by owner", async function () {
      const { ticketMarketplace, notOwner } = await loadFixture(
        deployMarketplaceERC20
      );
      await expect(
        ticketMarketplace.connect(notOwner).createEvent(1e5, 1e5, 1e5)
  ...
  describe("setMaxTicketsForEvent", function () {
    it("Should revert when not called by owner", async function () {
      const { ticketMarketplace, notOwner } = await loadFixture(
        marketplaceWithEvents
      );
      await expect(
        ticketMarketplace.connect(notOwner).setMaxTicketsForEvent(0, 1e10)
      ).to.be.revertedWith("Unauthorized access");
    });
    ...
    it("Should revert when the new maxTickets value is smaller than the one that is already present", async function () {
      ...
      await expect(
        ticketMarketplace.setMaxTicketsForEvent(0, maxTickets - 1n)
      ).to.be.revertedWith("The new number of max tickets is too small!");
    });
  ...
  describe("Price setters", function () {
    describe("setPriceForTicketETH", function () {
      it("Should revert when not called by owner", async function () {
        ...
        await expect(
          ticketMarketplace.connect(notOwner).setPriceForTicketETH(0, 1e10)
        ).to.be.revertedWith("Unauthorized access");
      });
    ...
    describe("setPriceForTicketERC20", function () {
      it("Should revert when not called by owner", async function () {
        ...
        await expect(
          ticketMarketplace.connect(notOwner).setPriceForTicketERC20(0, 1e10)
        ).to.be.revertedWith("Unauthorized access");
      });
      ...
      it("Should update the price", async function () {
        ...
        const newPrice = (await ticketMarketplace.events(0))
          .pricePerTicketERC20;
        await expect(newPrice).to.be.eq(1e10);
      });
    });
  ...
  describe("setERC20Address", function () {
    it("Should revert when not called by owner", async function () {
      ...
      await expect(
        ticketMarketplace
          .connect(notOwner)
          .setERC20Address("0x0000000000000000000000000000000000000001")
      ).to.be.revertedWith("Unauthorized access");
    });
  ...
  describe("Ticket buying methods", function () {
    describe("buyTickets", function () {
      it("Should NOT revert on a non-admin call", async function () {
        ...
        await expect(
          ticketMarketplace
            .connect(notOwner)
            .buyTickets(0, 1, { value: ethers.parseEther("1") })
        ).not.to.be.reverted;
      });

      it("Should revert on overflow", async function () {
        const { ticketMarketplace } = await loadFixture(marketplaceWithEvents);
        await ticketMarketplace.setPriceForTicketETH(
          0,
          ethers.parseUnits("1", 75)
        ); // 10 ** 75
        await expect(
          ticketMarketplace.buyTickets(0, ethers.parseUnits("1", 10))
        ).to.be.revertedWith(
      ...
      it("Should revert when not enough funds supplied", async function () {
        const { ticketMarketplace } = await loadFixture(marketplaceWithEvents);
        await expect(
          ticketMarketplace.buyTickets(0, 1, { value: 1 })
        ).to.be.revertedWith(
      ...
      it("Should revert when trying to buy too many tickets", async function () {
        const { ticketMarketplace } = await loadFixture(marketplaceWithEvents);
        await expect(
          ticketMarketplace.buyTickets(0, 1e6, {
            value: ethers.parseEther("1000"),
          })
        ).to.be.revertedWith("We don't have that many tickets left to sell!");
      });

      it("Should mint NFTs to user's account when all conditions are satisfied", async function () {
        const { owner, ticketMarketplace, ticketNFT } = await loadFixture(
          marketplaceWithEvents
        );
        await ticketMarketplace.buyTickets(1, 3, {
    ...
    describe("buyTicketsERC20", function () {
      it("Should NOT revert on a non-admin call", async function () {
        ...
        await expect(ticketMarketplace.connect(notOwner).buyTicketsERC20(0, 1))
          .not.to.be.reverted;
      });

      it("Should revert on overflow", async function () {
        const { ticketMarketplace } = await loadFixture(marketplaceWithEvents);
        await ticketMarketplace.setPriceForTicketERC20(
          0,
          ethers.parseUnits("1", 75)
        ); // 10 ** 75
        await expect(
          ticketMarketplace.buyTicketsERC20(0, ethers.parseUnits("1", 10))
        ).to.be.revertedWith(
          "Overflow happened while calculating the total price of tickets. Try buying smaller number of tickets."
        );
      });
      ...
      it("Should revert when trying to buy too many tickets", async function () {
        const { ticketMarketplace } = await loadFixture(marketplaceWithEvents);
        await expect(
          ticketMarketplace.buyTicketsERC20(0, 1e6)
        ).to.be.revertedWith("We don't have that many tickets left to sell!");
      });
  ...
  describe("Events", function () {
    ...
    it("Should emit an event after buying tickets", async function () {
      const { ticketMarketplace, owner } = await loadFixture(
        marketplaceWithEvents
      );
      ...
      await expect(
        ticketMarketplace
          .connect(owner)
          .buyTickets(0, 1, { value: ethers.parseEther("10") })
      )
    ...
    it("Should emit an event after changing ERC20 address", async function () {
      ...
      await expect(
        ticketMarketplace.setERC20Address(
          "0x0000000000000000000000000000000000000001"
        )
      )
  ...
  describe("Transfers", function () {
    it("Should transfer ETH to the contract after calling buyTickets", async function () {
      ...
      await expect(
        ticketMarketplace.buyTickets(0, 1, { value: ethers.parseEther("10") })
    ...
    it("Should transfer ERC20 tokens to the contract after calling buyTicketsERC20", async function () {
      const { notOwner, ticketMarketplace, sampleCoin } = await loadFixture(
        marketplaceWithEvents
      );

      await ticketMarketplace.connect(notOwner).buyTicketsERC20(0, 1);
      const balanceContract = await sampleCoin.balanceOf(
        await ticketMarketplace.getAddress()
      );
      const balanceNotOwner = await sampleCoin.balanceOf(
